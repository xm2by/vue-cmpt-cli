...
module.exports = prompt(questions).then(({ name, template, description, author }) => {
  ...
  download(repo, dest, (err) => {
    ...
    fs.readFile(path.join(__dirname, projectName, 'package.json'), 'utf-8', (err, data) => {
      ...
      fs.writeFile(path.join(__dirname, projectName, 'package.json'), newPackageJson, 'utf-8', (err) => {
        if (err) {
          spinner.stop()
          console.log(chalk.red(err))
          process.exit()
